...
export default async function ContentPage({
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
      ...
      <PageBody page={page as { id: number; type: string; track_id: number | null; content_module_id: number | null }} />
    </div>
  );
}
...
function PageBody({ page }: { page: { id: number; type: string; track_id: number | null; content_module_id: number | null } }) {
  switch (page.type) {
    case "plain-content":
      return <PlainContentRenderer pageId={page.id} />;
    case "text-lesson":
    case "audio-lesson":
      return <TextLessonRenderer pageId={page.id} />;
    case "h5p-quiz":
