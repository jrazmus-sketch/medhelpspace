...
export function ErrorsClient({ rows }: { rows: AppErrorRow[] }) {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-4">
      <div>
        <h1 className="text-xl font-bold">{t("appErrors.title")}</h1>
        <p className="max-w-[65ch] text-sm text-muted-foreground">{t("appErrors.subtitle")}</p>
      </div>
              ...
              <li key={r.id}>
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="mt-1 block text-xs text-muted-foreground">
                      {t("appErrors.count", { count: r.count })} · {t("appErrors.last", { when: fmt(r.last_seen) })}
                    </span>
                  </span>
                ...
                {isOpen ? (
                  <div className="space-y-2 border-t border-border bg-background/40 px-4 py-3 text-xs">
                    <p>
                      <span className="text-muted-foreground">{t("appErrors.first")}</span> {fmt(r.first_seen)}
                    </p>
                    {r.last_path ? (
                      <p className="break-all">
                        <span className="text-muted-foreground">{t("appErrors.path")}</span> {r.last_path}
                      </p>
                    ) : null}
                    {r.digest ? (
                      <p className="break-all">
                    ...
                    >
                      <Check className="h-4 w-4" /> {t("appErrors.resolve")}
                    </button>
                  </div>
                ) : null}
              </li>
