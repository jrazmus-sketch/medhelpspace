...
// (chooseFlashcardsCohortAndSend), never shown inline. Mirrors the funnel's capture
// discipline: honeypot + first-touch context, all writes via server actions.
...
export function FlashcardsGate({ utm }: { utm: MagnetUtm }) {
  ...
  function submitEmail() {
    ...
    if (!EMAIL_RE.test(em)) {
      setErr("Digite um e-mail válido para receber os flashcards.");
      return;
    }
    setErr(null);
    startTransition(async () => {
      const res = await captureFlashcardsLead({
        email: em,
        utm,
        honeypot: hp,
  ...
  function chooseCohort(slug: string) {
    ...
    setErr(null);
    startTransition(async () => {
      const res = await chooseFlashcardsCohortAndSend({
    ...
    return (
      <div className="rounded-2xl border border-brand/30 bg-surface-1/80 p-6 text-center shadow-[0_0_60px_-15px] shadow-brand/40 sm:p-8">
        ...
        <p className="mx-auto mt-2 max-w-sm text-sm text-muted-foreground">
          O link para os seus <strong className="text-foreground">50 flashcards</strong> foi para{" "}
          <strong className="text-foreground">{masked}</strong>. Abra o e-mail e toque em{" "}
          <span className="whitespace-nowrap font-medium text-brand">“Abrir meus 50 flashcards”</span>{" "}
          para começar.
        </p>
        {!emailed && (
  ...
  if (phase === "cohort") {
    return (
      <div className="rounded-2xl border border-brand/30 bg-surface-1/80 p-6 shadow-[0_0_60px_-15px] shadow-brand/40 sm:p-7">
        <p className="text-xs font-semibold uppercase tracking-wider text-brand">Última etapa</p>
        ...
        <p className="mt-1.5 text-sm text-muted-foreground">
          Assim personalizamos os lembretes e a sua condição de acesso à plataforma.
        </p>
