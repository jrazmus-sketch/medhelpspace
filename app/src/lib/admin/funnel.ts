...
// funnel_events only records the two PRE-capture steps of the quiz funnel
// ('landing' | 'quiz_start'). Everything from email capture onward comes from
// the leads table and is computed CLIENT-side in leads-client.tsx, so every
// number on the page shares one filter definition (test/archived exclusion,
// date range, funnel tab).
//
...
export async function getFunnelEventDays(): Promise<FunnelEventDay[]> {
  ...
  const [{ data: events }, { data: testLeads }] = await Promise.all([
    admin
      .from("funnel_events")
      .select("event_type, utm_source, created_at, session_id, is_internal")
      .limit(100000),
    admin
      .from("leads")
  ...
  for (const e of events ?? []) {
    ...
    const key = `${day}|${eventType}|${source ?? ""}|${internal ? 1 : 0}`;
    const existing = bucket.get(key);
    if (existing) existing.count++;
    else bucket.set(key, { day, eventType, source, internal, count: 1 });
  }
