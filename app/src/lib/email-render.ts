...
// Social profiles live in lib/social.ts — one list shared with the marketing
// footer, so a handle can never drift between the site and the e-mail.
...
function renderFooter(settings: EmailSettingsRow, unsubscribeUrl?: string): string {
  ...
  // Follow row: one glyph per network, then a single shared tagline beneath.
  //
  // The glyphs are hosted PNGs served from /public/brand — e-mail clients (Gmail,
  // Outlook) strip inline SVG and reject data: URIs, so each logo must be a real
  // image at an absolute URL. Generated by scripts/build-social-email-icons.js.
  //
  // ONE table row of icons rather than one row per network: three stacked
  // handle-plus-tagline lines made the footer four lines taller on a phone. The
  // tagline is shared because the handles are NOT all the same (TikTok carries
  // `_revalida`), so no single handle could honestly stand for all three.
  const socialIcons = SOCIAL_PROFILES.map(
    (p) => `<td style="padding-right:10px;vertical-align:middle;line-height:0;">
          <a href="${p.url}" style="text-decoration:none;">
            <img src="${settings.app_url}/brand/${p.emailIcon}" alt="${p.label}" width="20" height="20"
              style="display:block;border:0;outline:none;text-decoration:none;width:20px;height:20px;" />
          </a>
        </td>`,
  ).join("");

  const socialLine = `<table cellpadding="0" cellspacing="0" role="presentation" style="margin:0 0 4px;">
      <tr>${socialIcons}</tr>
    </table>
    <p style="margin:0 0 12px;font-size:12px;color:#9ca3af;line-height:1.5;">${SOCIAL_TAGLINE}</p>`;
